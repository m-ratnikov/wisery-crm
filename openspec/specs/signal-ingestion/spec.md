...
The ingestion spine and top of the funnel (D3): durable Sources as config-as-data, each scanned as an independent run that yields deduped, immutable Signals traceable to their source and scan. Sources plug in through one normalize-at-the-edge connector contract and are scanned one job per source for failure isolation, so adding a source type is a new connector, not a new pipeline.
