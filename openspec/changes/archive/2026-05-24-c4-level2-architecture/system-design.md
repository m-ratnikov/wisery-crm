...
in-process via pg-boss)<!-- v:derives ADR-0001 --> and introduces ADR-0002 (headless-browser scraping runs as an on-demand child
process the worker spawns - an extension of ADR-0001's in-process default for the one heavy dependency that cannot share the event loop).<!-- v:derives ADR-0002 --> The L1 section
...
Containers (C4 L2) section and the container-level runtime flows. As a verbatim snapshot, the
reproduced L1 still carries labels this change supersedes - the provider edge reads "normalized
records / enrichment" (this change makes normalization our edge connector's job, D4, so the provider
actually returns raw provider-shaped records), the LLM box reads "LLM API - Anthropic" (now
provider-neutral, D9), and the qualify/draft split is left open (now resolved: the draft is a separate
post-enrichment call, D5). All three are reconciled into the promoted system-context.md at promotion
(tasks.md section 2.3).<!-- v:decision - L1 snapshot carries pre-decision labels; reconciled at promotion -->
...
boxes.<!-- v:derives ADR-0001 --> Their split into web/worker roles, the optional `worker_threads` pool, and the on-demand
headless-browser child the worker spawns for browser-needing scrapes are L3 internals, deliberately
not drawn.<!-- v:decision --> Internal containers (what we build and run) are blue; external systems we depend on but do not
control are grey; the one optional external unit and its edge (the provider) are dashed. A legend
follows the diagram.
...
axes are independent: the provider, for instance, is external-and-optional.
...
ALTERNATIVES selected by the D4 cost/ban-risk knob, not concurrent.<!-- v:derives D4 --> The app's outbound
...
- **Headless browser via in-process Playwright (separate OS process, on demand)** (ADR-0002): chose to drive Playwright in-process - `launch()` spawns and supervises the browser (Chromium) as a separate OS process over a pipe/CDP, `close()` reaps it per scrape - over both running the browser on the worker's event loop and standing up a separate scraper service (`launchServer()` ws endpoint). The scraping engine and all API/HTTP fetching stay in-process; only the heavy, crashy browser (memory, own lifecycle, wide crash blast radius) gets its own OS process, so a crash surfaces as a rejected promise / `disconnected` event and fails only the one scrape (retried and dead-lettered by the job). It is genuinely optional (the D4 self-host path) and absent when only the provider or public APIs are used; the managed provider is the default for hardened/authenticated targets (keeping ban risk off the user's account, D2); Playwright over Puppeteer. Trade-off accepted (not zero-ops): concurrent browsers are capped by a semaphore sized to host memory (like the pg-boss pool budget), with try/finally `close()` + a hard timeout + an orphan reaper to bound the OOM-on-leak risk on the shared host; `newContext()` reuse, then a `launchServer()` pool or a dedicated host, are later scaling levers, not the default.<!-- v:decision; derives ADR-0002 -->
...
- Secrets: the enrichment-provider and LLM-provider API keys and the Postgres connection strings live only server-side in the app container (its web and worker roles) via `server-only`; on the self-host browser path, scraping session cookies live in the browser's OS process on the worker's host, not a separate unit (ADR-0002). The browser is a thin client and no secret-bearing path crosses to it.<!-- v:derives ADR-0002, server-only (CLAUDE.md) -->
- Failure handling: external calls run as pg-boss jobs with retries and a dead-letter path; one scan job per Source isolates a failing source from the others (ADR-0001); CPU-bound parsing, if profiling shows it, is isolated on the `worker_threads` pool; a headless-browser crash is contained in its own OS process and fails only that scrape; qualify and the draft-after-enrich are separate jobs, each idempotent with its own prompt_version, so a retry of one does not re-bill the other. On shutdown the app owns a SIGTERM/SIGINT handler that calls `boss.stop({ graceful: true })` within the host stop window; because a restart re-runs in-flight jobs on retry, externally-billed steps (provider, LLM) are idempotent (ADR-0001).<!-- v:derives ADR-0001 -->
