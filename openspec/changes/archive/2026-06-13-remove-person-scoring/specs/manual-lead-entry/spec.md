# manual-lead-entry - delta

Adding a lead creates the person and nothing more - there is no qualification to
enqueue, so the failed-enqueue recovery requirement disappears with it.
...
**Reason**: There is no qualification enqueue anymore (and no qualification at all); the failure mode this recovered from cannot occur.
**Migration**: None.
