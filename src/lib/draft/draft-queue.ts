...
export async function enqueueDraft(personId: string, force = false): Promise<string | null> {
  return enqueue(DRAFT_QUEUE, { personId, force }, { singletonKey: personId });
}
...
export async function enqueueDraftInTx(
  ...
  return enqueueInTx(tx, DRAFT_QUEUE, { personId, force }, { singletonKey: personId });
}
...
export async function registerDraftWorker(): Promise<void> {
  ...
  await work<{ personId: string; force?: boolean }>(DRAFT_QUEUE, async (jobs) => {
    for (const job of jobs) {
      await draftProspect(job.data.personId, { force: job.data.force });
    }
  });
}
