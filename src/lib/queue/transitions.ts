...
import { drafts, outcomes, prospects, scorings } from "@/lib/db/schema";
...
async function transitionFromQueued(
  ...
  const updated = await getDb()
    .update(prospects)
    .set({ status: to })
    .where(and(eq(prospects.id, prospectId), eq(prospects.status, "queued")))
    .returning({ id: prospects.id });
  return { prospectId, changed: updated.length > 0 };
}

export function actProspect(prospectId: string): Promise<TransitionOutcome> {
  return transitionFromQueued(prospectId, "acted");
}

export function dismissProspect(prospectId: string): Promise<TransitionOutcome> {
  return transitionFromQueued(prospectId, "dismissed");
}
...
export async function logOutcome(
  ...
  return getDb().transaction(async (tx) => {
    const closed = await tx
      .update(prospects)
      .set({ status: "closed" })
      .where(and(eq(prospects.id, prospectId), eq(prospects.status, "acted")))
      .returning({ id: prospects.id });
    if (closed.length === 0) {
      return { prospectId, changed: false };
    }
    ...
    if (!score) {
      throw new Error(`logOutcome: acted prospect ${prospectId} has no scoring`);
    }
