...
import { createSource, setSourceEnabled, updateSourceConfig } from "@/lib/signals/sources";
import { enqueueScan } from "@/lib/signals/scan-queue";
import { buildAndValidateConfig } from "@/lib/signals/source-kinds";
...
export async function createSourceAction(formData: FormData): Promise<void> {
  // Per-kind, validated connect (source-connection). buildAndValidateConfig builds the config
  // from the kind's declared fields and rejects an unknown/unregistered kind or a config the
  // kind's schema rejects - so a source whose scan would be dead-on-arrival, or whose config
  // is unusable, is never created (defense in depth: holds for a direct POST, not just the wizard).
  const kind = field(formData, "kind").trim();
  const config = buildAndValidateConfig(kind, (f) => field(formData, f));
  await createSource({ kind, config });
  revalidatePath(ROUTE);
}

export async function updateSourceSettingsAction(formData: FormData): Promise<void> {
  // Edit a connected source's settings in place, validated against its kind's schema (same
  // boundary as connect). The source keeps its id and history.
  const id = field(formData, "id");
  const kind = field(formData, "kind").trim();
  const config = buildAndValidateConfig(kind, (f) => field(formData, f));
  await updateSourceConfig(id, config);
  revalidatePath(ROUTE);
}
